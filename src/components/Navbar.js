...
function Navbar() {
  ...
  function toggletheme() {
    setdark(!dark)
    if(!dark) localStorage.theme = "light";
      ...
      <div className="flex justify-between relative">
        ...
        <div className="flex items-center h-16 gap-4 md:gap-6 pr-3 text-2xl md:text-3xl">
          <div className="lg:hidden hover:cursor-pointer">
            {" "}
            <FaBars className="md:text-2xl" />
          </div>
          <div className="relative hover:cursor-pointer">
            <IoNotificationsOutline />
            <div className="absolute -right-1 -top-1 outline outline-1 outline-gray-950 bg-purple-400 h-4 md:h-5 aspect-square rounded-full text-sm flex justify-center items-center">
              8
            </div>
          </div>
